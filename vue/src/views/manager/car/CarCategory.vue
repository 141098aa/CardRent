<template>
  <div class="admin-scope">
    ...
    <el-dialog
      ...
      :title="data.dialogType === 'add' ? '新增分类' : '编辑分类'"
      width="450px"
      ...
      <el-form ref="formRef" :model="data.form" :rules="data.rules" label-width="80px">
        ...
        <el-form-item label="分类编码" prop="code">
          <el-input v-model="data.form.code" placeholder="请输入分类编码，如: SUV" />
        </el-form-item>
...
const save = () => {
  formRef.value?.validate((valid) => {
    ...
    data.saving = true
    const apiCall = data.dialogType === 'add' ? carApi.addCategory : carApi.updateCategory
...
const handleDelete = (id) => {
  ElMessageBox.confirm('确定要删除该分类吗？删除后可能影响相关车辆', '提示', {
    type: 'warning',
    confirmButtonText: '确定',
    cancelButtonText: '取消'
  })
    .then(() => {
      carApi.deleteCategory(id).then((res) => {
        if (res.code === '200') {
...
const batchDelete = () => {
  ...
  ElMessageBox.confirm(`确定要删除选中的 ${selectedIds.value.length} 个分类吗？`, '提示', {
    type: 'warning',
    confirmButtonText: '确定',
    cancelButtonText: '取消'
  })
    .then(() => {
      Promise.all(selectedIds.value.map((id) => carApi.deleteCategory(id))).then(() => {
...
const updateSort = (row) => {
  carApi.updateCategory(row).then((res) => {
    if (res.code === '200') {
